...
uint8_t BCHG_Valid_EA[64] = {	1, 1, 1, 1, 1, 1, 1, 1,		// 000 xxx Dn
...
void mnemo_BCHG( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	uint16_t EA_Mode = M68k_Context_p->Current_Opcode & 0x003F;
	if( BCHG_Valid_EA[EA_Mode] == 0)
	{
	...
	{
		uint8_t Test_Value = EA_Read_B_KeepIntact( M68k_Context_p, N_Ticks, EA_Mode);
		if( M68k_Context_p->Interruptions != 0)
			return;
		...
		EA_Write_B_Table[EA_Mode]( M68k_Context_p, N_Ticks, EA_Mode & 7, Test_Value);
	}
