...
uint8_t CMP_Valid_EA[64] = {	1, 1, 1, 1, 1, 1, 1, 1,		// 000 xxx Dn
...
void mnemo_CMP_B( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint8_t Source = EA_Read_B_Table[EA_Mode](M68k_Context_p, N_Ticks, EA_Mode & 7);
	if( M68k_Context_p->Interruptions != 0)
		return;

	uint8_t Destination = M68k_Context_p->D[ (M68k_Context_p->Current_Opcode & 0x0E00) >> 9 ];
...
void mnemo_CMP_W( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint16_t Source = EA_Read_W_Table[EA_Mode](M68k_Context_p, N_Ticks, EA_Mode & 7);
	if( M68k_Context_p->Interruptions != 0)
		return;

	uint16_t Destination = M68k_Context_p->D[ (M68k_Context_p->Current_Opcode & 0x0E00) >> 9 ];

	M68k_Context_p->Status_Register &= 0xFFF0;
	M68k_Context_p->Status_Register |= CMP_W( Destination, Source);
...
void mnemo_CMP_L( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint32_t Source = EA_Read_L_Table[EA_Mode](M68k_Context_p, N_Ticks, EA_Mode & 7);
	if( M68k_Context_p->Interruptions != 0)
		return;

	uint32_t Destination = M68k_Context_p->D[ (M68k_Context_p->Current_Opcode & 0x0E00) >> 9 ];

	M68k_Context_p->Status_Register &= 0xFFF0;
	M68k_Context_p->Status_Register |= CMP_L( Destination, Source);
