...
uint8_t ADDI_Valid_EA[64] = {	1, 1, 1, 1, 1, 1, 1, 1,		// 000 xxx Dn
...
void mnemo_ADDI_B( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint8_t Result = Imm_Value + EA_Value;
	...
	if( (Imm_Value && EA_Value) || (Imm_Value && !Result) || (EA_Value && !Result))
...
void mnemo_ADDI_W( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint16_t Result = Imm_Value + EA_Value;
...
void mnemo_ADDI_L( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint32_t Result = Imm_Value + EA_Value;
