...
uint8_t DIVS_Valid_EA[64] = {	1, 1, 1, 1, 1, 1, 1, 1,		// 000 xxx Dn
...
void mnemo_DIVS( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	uint16_t EA_Mode = M68k_Context_p->Current_Opcode & 0x003F;
	if( DIVS_Valid_EA[EA_Mode] == 0)
	{
	...
	int32_t Remainder = Destination % Source;
