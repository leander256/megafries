...
uint8_t ADDQ_Valid_EA[64] = {	1, 1, 1, 1, 1, 1, 1, 1,		// 000 xxx Dn
...
void mnemo_ADDQ_B( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	...
	uint8_t Result = Source_Value + Destination_Value;
	...
	if( (EA_Mode & 0x0038) != 0x0008)	// if dest is not An
	{
		...
		if( (Result & 0x80) != 0)
		{
			...
			if( (Destination_Value & 0x80) == 0)		// simplified V formula since Source is always between 0 and 8
...
void mnemo_ADDQ_W( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	if( (M68k_Context_p->Current_Opcode & 0x0038) == 0x0008)
	{
		mnemo_ADDQ_L( M68k_Context_p, N_Ticks);
		return;
	}
	...
	uint16_t Result = Source_Value + Destination_Value;
	...
	if( (EA_Mode & 0x0038) != 0x0008)	// if dest is not An
	{
		...
		if( (Result & 0x8000) != 0)
		{
			...
			if( (Destination_Value & 0x8000) == 0)		// simplified V formula since Source is always between 0 and 8
				CCR_SET_V(M68k_Context_p->Status_Register);
		}
		else
		{
...
void mnemo_ADDQ_L( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	uint16_t EA_Mode = M68k_Context_p->Current_Opcode & 0x003F;
	if( ADDQ_Valid_EA[EA_Mode] == 0)
	{
	...
	uint32_t Result = Source_Value + Destination_Value;
	...
	if( (EA_Mode & 0x0038) != 0x0008)	// if dest is not An
	{
		...
		if( (Result & 0x80000000) != 0)
		{
			...
			if( (Destination_Value & 0x80000000) == 0)		// simplified V formula since Source is always between 0 and 8
				CCR_SET_V(M68k_Context_p->Status_Register);
		}
		else
		{
