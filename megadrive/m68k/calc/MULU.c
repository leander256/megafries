...
uint8_t MULU_Valid_EA[64] = {	1, 1, 1, 1, 1, 1, 1, 1,		// 000 xxx Dn
...
void mnemo_MULU( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	uint16_t EA_Mode = M68k_Context_p->Current_Opcode & 0x003F;
	if( MULU_Valid_EA[EA_Mode] == 0)
	{
	...
	uint32_t Destination = EA_Read_W_Dn( M68k_Context_p, N_Ticks, Reg_Number);
