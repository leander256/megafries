...
const uint8_t PEA_Valid_EA[] = {	2, 2, 2, 2, 2, 2, 2, 2,		// 000 xxx Dn
...
void mnemo_PEA( struct M68k_Context* M68k_Context_p, int32_t* N_Ticks)
{
	uint16_t EA_Mode = M68k_Context_p->Current_Opcode & 0x003F;

	switch( PEA_Valid_EA[EA_Mode])
	{
		case 0:	M68k_Context_p->Interruptions |= INT_ILLEGAL;
				return;
		case 2: mnemo_SWAP( M68k_Context_p, N_Ticks);
				return;
	}
